...
\begin{align}
	V_{out} = A(V_+-V_-).
\end{align}
$A$ is a very large number, so in practice $V_{out}$ can get only values 9V and -9V.
...
The circuit we made and used is basically a voltage amplifier built around LF356N operational amplifier and later it is modified to current-voltage converter by replacing $R_1$-resistor with quartz resonator and changing $R_f$-resistor to a approximately $\SI{1}{\mega\ohm}$-resistor. The derivation of $K$ (equation \eqref{eqn:K}) is done in the appendix as part of a pretask.

To measure quartz resonator the circuit must be connected to a DAQ-unit as shown in figure \ref{fig:circ} and then it can be measured by setting the DAQ-unit to output a sine wave with certain frequency and then measuring the output of our circuit. With multiple measurements a spike in gain is expected at the resonance frequency of the quartz resonator as seen in figure \ref{fig:f-g}.

\begin{figure}[!ht]
\centering
\includegraphics[width = \textwidth]{kuvat/f-g.png}
...
&=R_m+\frac{1}{i\omega \frac{1}{\omega^2L_m}} + i\omega L_m\\
&=R_m -i\omega L_m + i\omega L_m = R_m
\end{align*}
\end{document}